...
import { ArrowLeft, Landmark, MapPin, Plane, Sparkles, Heart, Bookmark, CheckCircle } from 'lucide-react'
...
export default function CountryDetailPage() {
  ...
  const { currentUser, isAuthenticated, toggleFavorite, addToWishlist, markVisited } = useAuth()
  const country = useMemo(() => countries.find((item) => item.id === id), [id])
  const isFavorite = currentUser?.favoriteCountries?.includes(id)
  const isInWishlist = currentUser?.wishlist?.some((item) => item.id === id)
  const isVisited = currentUser?.visitedCountries?.includes(id)
    ...
    <div className="pb-20">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
          <button
            type="button"
            onClick={() => requireAuth(() => toggleFavorite(country))}
            className={`inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-semibold transition ${isFavorite ? 'bg-rose-500 text-white hover:bg-rose-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
          >
          ...
          <button
            type="button"
            onClick={() => requireAuth(() => addToWishlist(country))}
            className={`inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-semibold transition ${isInWishlist ? 'bg-sky-600 text-white hover:bg-sky-700' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
          >
          ...
          <button
            type="button"
            onClick={() => requireAuth(() => markVisited(id))}
            className={`inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-semibold transition ${isVisited ? 'bg-emerald-500 text-white hover:bg-emerald-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
          >
      ...
      {country.festivals?.length ? (
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <SectionTitle eyebrow="Festivals" title="Events and celebrations" description="Discover the festivals and cultural celebrations that make this country special." />
          <div className="mt-8 rounded-[28px] border border-slate-200 bg-white p-8 shadow-sm">
            <div className="grid gap-4 sm:grid-cols-3">
              {country.festivals.map((festival) => (
                <div key={festival} className="rounded-3xl border border-slate-100 bg-slate-50 px-5 py-4 text-sm font-medium text-slate-800 shadow-sm">
                  {festival}
                </div>
              ))}
            </div>
